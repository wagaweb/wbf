...
namespace WBF\includes\theme_customizations;
 ...
 * @return string
 */
function alter_wp_title( $title, $sep ) {
...
add_filter( 'wp_title', __NAMESPACE__.'\\alter_wp_title', 10, 2 );
...
add_filter('sanitize_file_name', __NAMESPACE__.'\\sanitize_file_name_chars', 10);

/*
 * EMAIL ENCODING
 * Based on Email Address Encoder by @Till Krüss - https://encoder.till.im/
 */
if(!defined('WBF_EMAIL_ENCODING_ENABLED')){
	define('WBF_EMAIL_ENCODING_ENABLED', true);
}

if (!defined('WBF_EMAIL_ENCODING_FILTER_PRIORITY')){
	define('WBF_EMAIL_ENCODING_FILTER_PRIORITY', 1000);
}

if(WBF_EMAIL_ENCODING_ENABLED){
	foreach (['the_content', 'the_excerpt', 'widget_text', 'comment_text', 'comment_excerpt'] as $filter) {
		add_filter($filter, __NAMESPACE__.'\\encode_emails', WBF_EMAIL_ENCODING_FILTER_PRIORITY);
	}
}
...
function encode_emails($string) {
	...
	return preg_replace_callback( $regexp, function ( $matches ) {
		return encode_email_str( $matches[0] );
	}, $string );
}
...
function encode_email_str($string) {
	...
	foreach ($chars as $key => $char) {
		...
		if ($ord < 128) { // ignore non-ascii chars
			...
			if ($r > 60 && $char != '@'){
				continue; // plain character (not encoded), if not @-sign
			}elseif($r < 45){
